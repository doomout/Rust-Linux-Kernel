...
- 빌더 패턴
    - 복잡한 파라미터를 가진 객체를 빌더를 사용해 쉽게 만들 수 있도록 하는 패턴
    - 복잡한 객체 생성을 단계적으로 구성하여 메서드 체이닝 방식으로 가독성을 높인다.
    - build()를 통해 객체 생성을 마무리 함으로 객체의 불변성을 보장한다.
- 어댑터 패턴
    - 서로 호환되지 않는 인터페이스를 연결해주는 패턴
    - 기존 코드는 바꾸기 어렵지만, 원하는 방식을 새 인터페이스(Adapter) 로 감싸서 사용

## 22. 매크로  
- 매크로: 코드를 단순화 하고 재사용하기 위한 방법 중 하나로 일종의 단축 명령을 의미
- 함수와 매크로의 차이  

| 항목	        | 함수	                      | 매크로                                   |
|---------------|----------------------------|------------------------------------------|
| 실행 시점	    | 런타임	                  | 컴파일 타임                               |
| 파라미터 처리	| 런타임에 지역변수를 생성     | 컴파일 타임에 소스코드 본문에 추가        |
...
|항목	|공유 메모리 (Shared Memory)	|메모리 맵 파일 (mmap)|
|----------|------------------------------|-------------------------|
|개념	|프로세스 간에 메모리 공간을 공유	|파일 내용을 메모리에 매핑|
|용도	|주로 IPC(프로세스 간 통신)	|파일 입출력 최적화 또는 IPC |
|지원 크레이트	|shmem-ipc, shared_memory 등	|표준 라이브러리(libc), memmap2 등 |
|파일 필요 	|파일 없이 메모리 영역 공유	|파일 또는 익명 파일 필요 |
|속도	|매우 빠름 (메모리 직접 접근)	|빠름 (디스크 캐시 활용) |
|생명 주기	|커널에서 관리되며 명시적 해제가 필요	|파일이 삭제되면 사라짐 (익명 mmap 제외) |
|Rust 난이도	|비교적 복잡 (unsafe + FFI 많음)	|memmap2로 비교적 단순하게 가능 |
|사용 예시	|프로세스 간 대용량 데이터 전달	|데이터베이스, 이미지/바이너리 파일 읽기 최적화 |
